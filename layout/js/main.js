    ...
    // Blogger Page
        if(document.title == 'المدونة'){
            ...
            if(window.innerWidth <= 576){
                ...
                let bloggerImage= document.querySelectorAll(".body-post img");

                for(let i= 0; i <= bloggerImage.length ; i++){

                    let getAttr= bloggerImage[i].getAttribute("src");
                    let arr= getAttr.split(".");
                    let str = arr[0] + "." + arr[1] + "-mobile" + "." + arr[2];
