<?php
    $title ='سيراميك ريماس';
    $style_name ="layout/css/ceramic/ceramic-remas.css";
    include "init.php";
?>
<section class="ceramic-remaas">
        <div class="header">
            <img src="layout/images/ceramic/remas/header.jpg" alt="Cl floor">
            <h2>ريماس</h2>
        ...
        <h4 class="text-right">التصنيف</h4>
            <p class="text-right">ارضيات / ريماس</p>
            <div class="row">
            <?php
                $stmt=$con->prepare("SELECT * FROM ceramic WHERE CompanyName=? AND type=?");
                $stmt->execute(array('remas',"floor"));
                $row=$stmt->fetchAll();
                foreach ($row as $rows) {
            ?>
                    <div class="card" >
                        <img src="<?php echo 'layout/images/ceramic/remas/' . $rows['Image']; ?>" class="card-img-top" alt="<?php echo $rows['Image']; ?>">
                        <div class="card-body">
            ...
            <h4 class="text-right">التصنيف</h4>
            <p class="text-right">مطابخ / ريماس</p>
            <div class="row">
            <?php
                $stmt=$con->prepare("SELECT * FROM ceramic WHERE CompanyName=? AND type=?");
                $stmt->execute(array('remas',"kitchen"));
                $row=$stmt->fetchAll();
                foreach ($row as $rows) {
            ?>
                    <div class="card" >
            ...
            <h4 class="text-right">التصنيف</h4>
            <p class="text-right">حمامات / ريماس</p>
            <div class="row">
            <?php
                $stmt=$con->prepare("SELECT * FROM ceramic WHERE CompanyName=? AND type=?");
                $stmt->execute(array('remas',"bathroom"));
                $row=$stmt->fetchAll();
                foreach ($row as $rows) {
            ?>
                    <div class="card" >
